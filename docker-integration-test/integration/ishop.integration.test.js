...
describe('iShop Integration Tests', () => {
  ...
  test('POST /products creates a product', async () => {
    const res = await request(app).post('/api/v1/products').send({ name: 'Test Product', price: 9.99, stock: 10 });
  ...
  test('PUT /products/:id updates a product', async () => {
    const res = await request(app).put(`/api/v1/products/${createdProductId}`).send({ name: 'Updated Product', price: 19.99, stock: 5 });
    expect(res.statusCode).toBe(200);
  ...
  test('POST /products with invalid data returns 400', async () => {
    const res = await request(app).post('/api/v1/products').send({ price: -1 });
    expect(res.statusCode).toBe(400);
  });
  test('PUT /products/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/products/999999').send({ name: 'x', price: 1, stock: 1 });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /products/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/products/1').send({ price: -100 });
    expect(res.statusCode).toBe(400);
  });
  ...
  test('POST /users creates a user', async () => {
    const res = await request(app).post('/api/v1/users').send({ username: 'testuser', password: 'password123', email: 'test@example.com' });
    expect(res.statusCode).toBe(201);
  ...
  test('PUT /users/:id updates a user', async () => {
    const res = await request(app).put(`/api/v1/users/${createdUserId}`).send({ username: 'updateduser', email: 'updated@example.com' });
    expect(res.statusCode).toBe(200);
  ...
  test('POST /users with missing fields returns 400', async () => {
    const res = await request(app).post('/api/v1/users').send({ username: '' });
    expect(res.statusCode).toBe(400);
  });
  test('PUT /users/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/users/999999').send({ username: 'x' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /users/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/users/1').send({ email: 'not-an-email' });
  ...
  test('POST /workers creates a worker', async () => {
    const res = await request(app).post('/api/v1/worker').send({ name: 'Worker1', role: 'packer' });
    expect(res.statusCode).toBe(201);
    createdWorkerId = res.body.id;
  });
  test('GET /workers returns workers', async () => {
    const res = await request(app).get('/api/v1/worker');
    expect(res.statusCode).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  });
  test('GET /workers/:id returns a single worker', async () => {
    const res = await request(app).get(`/api/v1/worker/${createdWorkerId}`);
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty('id', createdWorkerId);
  });
  test('PUT /workers/:id updates a worker', async () => {
    const res = await request(app).put(`/api/v1/worker/${createdWorkerId}`).send({ name: 'Worker2', role: 'shipper' });
    expect(res.statusCode).toBe(200);
    expect(res.body.name).toBe('Worker2');
  });
  test('DELETE /workers/:id deletes a worker', async () => {
    const res = await request(app).delete(`/api/v1/worker/${createdWorkerId}`);
    expect(res.statusCode).toBe(204);
  });
  // WORKERS ERROR CASES
  test('GET /workers/:id with invalid id returns 404', async () => {
    const res = await request(app).get('/api/v1/worker/999999');
    expect(res.statusCode).toBe(404);
  });
  test('POST /workers with missing fields returns 400', async () => {
    const res = await request(app).post('/api/v1/worker').send({ name: '' });
    expect(res.statusCode).toBe(400);
  });
  test('PUT /workers/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/worker/999999').send({ name: 'x', role: 'x' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /workers/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/worker/1').send({ name: '', role: '' });
    expect(res.statusCode).toBe(400);
  });
  test('DELETE /workers/:id with non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/v1/worker/999999');
    expect(res.statusCode).toBe(404);
  });

  // ADMIN
  test('POST /admin creates an admin', async () => {
    const res = await request(app).post('/api/v1/admin').send({ username: 'admin1', password: 'adminpass' });
    expect(res.statusCode).toBe(201);
  ...
  test('GET /admin/:id returns a single admin', async () => {
    const res = await request(app).get(`/api/v1/admin/${createdAdminId}`);
    expect(res.statusCode).toBe(200);
    expect(res.body).toHaveProperty('id', createdAdminId);
  });
  test('PUT /admin/:id updates an admin', async () => {
    const res = await request(app).put(`/api/v1/admin/${createdAdminId}`).send({ username: 'admin2' });
    expect(res.statusCode).toBe(200);
    expect(res.body.username).toBe('admin2');
  });
  test('DELETE /admin/:id deletes an admin', async () => {
    const res = await request(app).delete(`/api/v1/admin/${createdAdminId}`);
    expect(res.statusCode).toBe(204);
  });
  // ADMIN ERROR CASES
  test('GET /admin/:id with invalid id returns 404', async () => {
    const res = await request(app).get('/api/v1/admin/999999');
    expect(res.statusCode).toBe(404);
  });
  test('POST /admin with missing fields returns 400', async () => {
    const res = await request(app).post('/api/v1/admin').send({ username: '' });
    expect(res.statusCode).toBe(400);
  });
  test('PUT /admin/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/admin/999999').send({ username: 'x' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /admin/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/admin/1').send({ username: '' });
    expect(res.statusCode).toBe(400);
  });
  test('DELETE /admin/:id with non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/v1/admin/999999');
    expect(res.statusCode).toBe(404);
  });

  // CATEGORIES
  test('POST /categories creates a category', async () => {
    const res = await request(app).post('/api/v1/categories').send({ name: 'Electronics' });
    expect(res.statusCode).toBe(201);
  ...
  test('PUT /categories/:id updates a category', async () => {
    const res = await request(app).put(`/api/v1/categories/${createdCategoryId}`).send({ name: 'Updated Category' });
    expect(res.statusCode).toBe(200);
  ...
  test('PUT /categories/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/categories/999999').send({ name: 'x' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /categories/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/categories/1').send({ name: '' });
    expect(res.statusCode).toBe(400);
  });
  test('DELETE /categories/:id with non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/v1/categories/999999');
    expect(res.statusCode).toBe(404);
  });
  ...
  test('POST /payments creates a payment', async () => {
    ...
    const userRes = await request(app).post('/api/v1/users').send({ username: 'payuser', password: 'pass', email: 'pay@example.com' });
    const productRes = await request(app).post('api/v1/products').send({ name: 'Pay Product', price: 10, stock: 5 });
  ...
  test('PUT /payments/:id updates a payment', async () => {
    const res = await request(app).put(`/api/v1/payments/${createdPaymentId}`).send({ method: 'paypal' });
    expect(res.statusCode).toBe(200);
    expect(res.body.method).toBe('paypal');
  });

  test('DELETE /payments/:id deletes a payment', async () => {
    const res = await request(app).delete(`/api/v1/payments/${createdPaymentId}`);
    expect(res.statusCode).toBe(204);
  });
  // PAYMENT ERROR CASES
  test('POST /payments with missing amount or currency returns 400', async () => {
    const res = await request(app).post('/api/v1/payments').send({ amount: 1000 }); // missing currency
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });

  test('POST /payments with invalid amount returns 400', async () => {
    const res = await request(app).post('/api/v1/payments').send({ amount: -1000, currency: 'usd', method: 'credit_card' });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });

  test('POST /payments with invalid currency returns 400', async () => {
    const res = await request(app).post('/api/v1/payments').send({ amount: 1000, currency: 'invalid', method: 'credit_card' });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });

  test('POST /payments with non-number amount returns 400', async () => {
    const res = await request(app).post('/api/v1/payments').send({ amount: "not-a-number", currency: 'usd', method: 'credit_card' });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });

  test('GET /payments/:id with invalid id returns 404', async () => {
    const res = await request(app).get('/api/v1/payments/999999');
    expect(res.statusCode).toBe(404);
  });
  test('PUT /payments/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/payments/999999').send({ method: 'cash' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /payments/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/payments/1').send({ method: '' });
    expect(res.statusCode).toBe(400);
  });
  test('DELETE /payments/:id with non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/v1/payments/999999');
    expect(res.statusCode).toBe(404);
  });

  // AUTH
  test('POST /auth/register registers a user', async () => {
    const res = await request(app).post('/api/v1/auth/register').send({ username: 'authuser', password: 'authpass', email: 'auth@example.com' });
    expect(res.statusCode).toBe(201);
  });
  test('POST /auth/login logs in a user', async () => {
    const res = await request(app).post('/api/v1/auth/login').send({ username: 'authuser', password: 'authpass' });
    expect(res.statusCode).toBe(200);
  ...
  test('POST /auth/register with missing fields returns 400', async () => {
    const res = await request(app).post('/api/v1/auth/register').send({ username: '' });
    expect(res.statusCode).toBe(400);
  });
  test('POST /auth/login with wrong credentials returns 401', async () => {
    const res = await request(app).post('/api/v1/auth/login').send({ username: 'authuser', password: 'wrongpass' });
  ...
  test('POST /orders creates an order', async () => {
    const productRes = await request(app).post('api/v1/products').send({ name: 'Order Product', price: 5.99, stock: 20 });
    const userRes = await request(app).post('api/v1/users').send({ username: 'orderuser', password: 'pass', email: 'order@example.com' });
    const order = {
      userId: userRes.body.id,
      items: [{ productId: productRes.body.id, quantity: 2 }]
    };
    const res = await request(app).post('/api/v1/orders').send(order);
    expect(res.statusCode).toBe(201);
  ...
  test('PUT /orders/:id updates an order', async () => {
    const res = await request(app).put(`/api/v1/orders/${createdOrderId}`).send({ status: 'shipped' });
    expect(res.statusCode).toBe(200);
    expect(res.body.status).toBe('shipped');
  });
  test('DELETE /orders/:id deletes an order', async () => {
    const res = await request(app).delete(`/api/v1/orders/${createdOrderId}`);
    expect(res.statusCode).toBe(204);
  });
  // ORDERS ERROR CASES
  ...
  test('PUT /orders/:id with non-existent id returns 404', async () => {
    const res = await request(app).put('/api/v1/orders/999999').send({ status: 'cancelled' });
    expect(res.statusCode).toBe(404);
  });
  test('PUT /orders/:id with invalid data returns 400', async () => {
    const res = await request(app).put('/api/v1/orders/1').send({ status: 123 });
    expect(res.statusCode).toBe(400);
  });
  test('DELETE /orders/:id with non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/v1/orders/999999');
    expect(res.statusCode).toBe(404);
  });

  // GENERAL ERROR CASES
  test('PATCH /products/:id returns 405 or 404', async () => {
    const res = await request(app).patch('api/v1/products/1').send({ name: 'patch' });
